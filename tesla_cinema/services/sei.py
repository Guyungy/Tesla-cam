...
def convert_to_data_points(
    ...
    The unit conventions below come from real-footage measurements and must stay
    in sync with the Electron implementation (``src/utils/parseSEI.ts``), which is
    the product's source of truth:

    - ``vehicleSpeedMps``: m/s; values above 90 are already km/h (see ``_to_speed_kph``)
    - ``acceleratorPedalPosition``: already a 0-100 percent, do NOT scale by 100
    ...
    for index, msg in enumerate(messages):
        ...
        points.append(
            SEIDataPoint(
                ...
                throttle_pct=min(
                    100.0, max(0.0, float(msg.get("acceleratorPedalPosition", 0.0)))
                ),
