...
def build_telemetry_frames(footage: CamFootage, export_start_seconds: float, export_duration_seconds: float) -> list[StrictTelemetryFrame]:
    ...
    for frame_index in range(frame_count):
        ...
        point = find_closest_sei(footage.sei_data, target)
...
def find_closest_sei(sei_data: list[SEIDataPoint], target: float) -> SEIDataPoint | None:
    return min(sei_data, key=lambda item: abs(item.offset_seconds - target)) if sei_data else None
...
def build_filter_graph(request: StrictExportRequest, icon_path: Path) -> tuple[str, list[str]]:
    ...
    if request.telemetry_frames:
        frame = request.telemetry_frames[0]
        overlays.extend(
            [
                "drawbox=x=0:y=0:w=iw:h=50:color=black@0.6:t=fill",
                f"drawtext=text='{escape_text(frame.timestamp_text)}':font='Segoe UI':fontsize=26:fontcolor=white:x=70:y={request.video_height + 24}",
                f"drawtext=text='{escape_text(f'{round(frame.speed_kph)} km/h')}':font='Segoe UI':fontsize=24:fontcolor=0x22c55e:x=(w-tw)/2:y=10",
                f"drawtext=text='{escape_text(frame.gear)}':font='Segoe UI':fontsize=20:fontcolor=white:x=24:y=12",
            ]
        )
        if frame.ap_status and frame.ap_status != "UNKNOWN":
            overlays.append(
                f"drawtext=text='{escape_text(frame.ap_status)}':font='Segoe UI':fontsize=12:fontcolor=white:x=w-tw-24:y=18"
            )
    overlays.append("[base]")
    overlays.append(f"[{icon_input}:v]scale=30:30[icon]")
    overlays.append(f"[base][icon]overlay=22:{request.video_height + 14}[outv]")
    filter_parts.append(",".join(overlays))
