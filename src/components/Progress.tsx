...
export function Progress({
  ...
  const hasRange =
    inPct !== undefined && outPct !== undefined && outPct > inPct;
  ...
  const updateTooltip = useCallback(
    (clientX: number) => {
      if (!containerRef.current || !tooltipRef.current || !hoverLineRef.current)
        return;
      const rect = containerRef.current.getBoundingClientRect();
  ...
  const speedCurvePath = useMemo(() => {
    ...
    const lastX =
      (speedData[Math.min(speedData.length - 1, (pts.length - 1) * step)]
        .offsetSeconds /
        max) *
      svgW;
  ...
  return (
    ...
    >
      ...
      <div
        ref={tooltipRef}
        className="pointer-events-none absolute -top-8 z-30 hidden -translate-x-1/2 rounded bg-neutral-800 px-2 py-0.5 text-[11px] font-medium text-white tabular-nums shadow"
      />
      ...
      <div className="relative h-2 w-full rounded-full bg-white/10 transition-all group-hover:h-8 group-hover:bg-white/5">
        ...
        <div
          ref={hoverLineRef}
          className="pointer-events-none absolute -top-1 -bottom-1 hidden w-px bg-white/40"
        />
      </div>
    </div>
  );
